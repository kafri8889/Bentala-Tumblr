...
object DateModule {
	...
	fun inSameMonth(t1: Long, t2: Long): Boolean {
		return YearMonth.from(Instant.ofEpochMilli(t1).atZone(ZoneId.systemDefault())).month == YearMonth.from(Instant.ofEpochMilli(t2).atZone(ZoneId.systemDefault())).month
	}
	...
	fun inSameYear(t1: Long, t2: Long): Boolean {
		return Year.from(Instant.ofEpochMilli(t1).atZone(ZoneId.systemDefault())).value == Year.from(Instant.ofEpochMilli(t2).atZone(ZoneId.systemDefault())).value
	}

	/**
	 * Check if two date in the same month and year
	 *
	 * @param t1 First date (time in milliseconds)
	 * @param t2 Second date (time in milliseconds)
	 */
	fun inSameMonthAndYear(t1: Long, t2: Long): Boolean {
		val ym1 = YearMonth.from(Instant.ofEpochMilli(t1).atZone(ZoneId.systemDefault()))
		val ym2 = YearMonth.from(Instant.ofEpochMilli(t2).atZone(ZoneId.systemDefault()))

		return ym1.year == ym2.year && ym1.month == ym2.month
	}

	fun calendarDayOfWeekToJavaTimeDayOfWeek(dayOfWeek: Int): DayOfWeek {
	...
	fun getWeek(millis: Long): Week {
		val calendarFirstDayOfWeek = Calendar.getInstance().apply {
			timeInMillis = millis
		}.firstDayOfWeek
		
		val date = Instant.ofEpochMilli(millis).atZone(ZoneId.systemDefault()).toLocalDate()
		val firstDayOfWeek = date.with(TemporalAdjusters.previousOrSame(calendarDayOfWeekToJavaTimeDayOfWeek(calendarFirstDayOfWeek)))
